...
public class Configs {
	...
	private static final String kConfigFolderName = "config",
			kLoggerTag = Util.classToJsonName(Configs.class);
	...
	private static Optional<String> getDefaultJson(Class<? extends ConfigBase> configClass) {
		try {
			return Optional.ofNullable(
					String.format(
							"See here for a default JSON file:%n%s%n",
	...
	private static <T extends ConfigBase> T read(Class<T> configClass) {
		...
		try {
			...
			Log.error(
					kLoggerTag, String.format("Error reading config %s", configClassName), readException);
			throw exception;
		}
	}
	...
	public static <T extends ConfigBase> boolean save(Class<T> configClass) {
		try {
			...
			Log.error(
					kLoggerTag,
					String.format("Could not save config %s", configClass.getSimpleName()),
					saveException);
			return false;
		}
	}
	...
	private static void notifyUpdated(Class<? extends ConfigBase> configClass) {
		// TODO nasty
		Optional.ofNullable(sListeners.get(configClass))
	...
	public static boolean reload(Class<? extends ConfigBase> configClass) {
		...
		try {
			...
			Log.error(
					kLoggerTag,
					String.format("Could not reload configs %s", configClass.getSimpleName()),
					exception);
		}
		return false;
	}
	...
	public static String toJson(Object object) {
		try {
			...
			Log.warn(
					kLoggerTag,
					String.format("Could not format %s as JSON", object.getClass().getSimpleName()),
					formatException);
	...
	public static <T> T copy(T toCopy) {
		try {
			...
			throw new IllegalArgumentException(
					"Type supplied cannot be represented as JSON! Can not copy.", exception);
		}
	}
				...
				try {
					...
					*/
					synchronized (sRobotThread) {
						sRobotThread.wait(
								100L); // In case something goes horribly wrong we can resume robot thread
						// execution
						for (WatchEvent<?> pollEvent : key.pollEvents()) {
							...
							if (configClass != null) {
								...
								try {
									...
									sMapper
											.readerForUpdating(config)
											.readValue(getFileForConfig(configClass).toFile());
									config.onPostUpdate();
								} catch (IOException readException) {
									RuntimeException exception = handleParseError(readException, configClass);
									Log.error(
											kLoggerTag,
											String.format("Error updating config for %s. Aborting reload.%n", configName),
											exception);
								}
								notifyUpdated(configClass);
