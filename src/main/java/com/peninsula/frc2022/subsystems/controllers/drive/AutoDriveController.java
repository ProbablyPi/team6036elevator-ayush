...
public class AutoDriveController extends Swerve.SwerveController {
	...
	private HolonomicDriveController mTrajectoryController = new HolonomicDriveController(
	...
	public void updateSignal(Commands commands, RobotState state) {
		...
		PathPlannerTrajectory mPathPlannerTrajectoryWanted = commands.getDriveWantedPathPlannerTrajectory();

		if (mPathPlannerTrajectory != mPathPlannerTrajectoryWanted) {
			mPathPlannerTrajectory = mPathPlannerTrajectoryWanted;

			ProfiledPIDController thetaController = new ProfiledPIDController(-2.0, 0.00, 0.00,
					new TrapezoidProfile.Constraints(4 * Math.PI, 4 * Math.PI));
			thetaController.enableContinuousInput(-Math.PI, Math.PI);

			mTrajectoryController = new HolonomicDriveController(
					new PIDController(2.14, 0.0, 0.0),
					new PIDController(1.94, 0.0, 0.0),
					thetaController);
			mTimer.reset();
		}
		PathPlannerTrajectory.PathPlannerState targetPose = (PathPlannerTrajectory.PathPlannerState) mPathPlannerTrajectory.sample(mTimer.get());

		ChassisSpeeds targetSpeeds = mTrajectoryController.calculate(
				state.driveOdometry.getPoseMeters(),
				targetPose,
				targetPose.holonomicRotation);

		SwerveModuleState[] moduleStates = SwerveConstants.kKinematics.toSwerveModuleStates(targetSpeeds);
