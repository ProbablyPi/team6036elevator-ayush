...
public class FieldRelativeAngleController extends Swerve.SwerveController {
	...
	public FieldRelativeAngleController() {
		angleSetPIDController = new PIDController(
	...
	public void updateSignal(Commands commands, RobotState state) {
		double x = state.driverLeftX * (commands.boostWanted ? SwerveConstants.kTeleopBoostMaxTransVel : SwerveConstants.kTeleopMaxTransVel);
		double y = state.driverLeftY * (commands.boostWanted ? SwerveConstants.kTeleopBoostMaxTransVel : SwerveConstants.kTeleopMaxTransVel);
		double z = angleSetPIDController.calculate(state.gyroHeading.getDegrees(), getAngle(commands, state));

		SwerveModuleState[] moduleStates = SwerveConstants.kKinematics.toSwerveModuleStates(
				commands.robotCentricWanted ? new ChassisSpeeds(-x, y, z) : ChassisSpeeds.fromFieldRelativeSpeeds(-x, y, z, state.gyroHeading));
	...
	public void normalize(SwerveModuleState[] swerveModuleStates) {
		...
		if (shouldNormalize) {
			for (SwerveModuleState state : swerveModuleStates) {
				state.speedMetersPerSecond *= SwerveConstants.kMaxVelocityMetersPerSecond / maxModuleVelocity;
			}
		}
	}
}
